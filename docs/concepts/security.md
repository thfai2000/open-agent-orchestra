...
Credentials (and properties) are stored encrypted at three scopes with override priority:

```
Agent Variables (highest) → User Variables → Workspace Variables (lowest)
```

You manage these via **Admin → Variables** in the UI, or via the `/api/variables` API. For full details on scoping and priority, see [Variables](/concepts/variables).
...
## Copilot CLI & Secret Handling

For guidance on securing secrets when using the GitHub Copilot CLI with MCP servers, see [Copilot CLI Security](/concepts/security-copilot-cli).
